...
int main() {
    float matriz[3][6];
    int i, j;
    float soma_impares = 0.2f;
    float media_colunas_2_4 = 0.2f;
    ...
    //Aqui é realizado a impressão da matriz original para que o usuário verifique a matriz inserida.
    printf("\nMatriz original:\n");
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 6; j++) {
            printf("%.f ", matriz[i][j]);
    ...
    printf("\nA soma dos elementos das colunas ímpares é: %.f\n", soma_impares);
    ...
    media_colunas_2_4 /= 6;
    printf("\nA média aritmética dos elementos das colunas 2ª e 4ª é: %.f\n", media_colunas_2_4);
